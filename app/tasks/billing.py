...
def generate_event_invoice(self, event_id: int):
    ...
    try:
        return _sync_generate_invoice(event_id)
    except Exception as e:
        logger.error(f"Failed to generate invoice for event {event_id}: {e}\n{traceback.format_exc()}")
        raise self.retry(exc=e, countdown=30)


def _sync_generate_invoice(event_id: int) -> dict:
    """Sync implementation of invoice generation."""
    with SyncSessionLocal() as db:
        ...
        existing = db.execute(
            select(PlatformInvoice).where(PlatformInvoice.event_id == event_id)
        ).scalar_one_or_none()
        ...
        if event.billing_profile_id:
            billing_profile = db.execute(
                select(OrganizerBillingProfile).where(
                    OrganizerBillingProfile.id == event.billing_profile_id,
                    OrganizerBillingProfile.is_active == True,
                )
            ).scalar_one_or_none()
            if billing_profile:
                logger.info(f"Using event's assigned billing profile {billing_profile.id} for event {event_id}")
        ...
        if not billing_profile:
            ...
            if billing_profile:
                logger.info(f"Using organizer's primary billing profile {billing_profile.id} for event {event_id}")
        ...
        if not billing_profile:
            ...
            if billing_profile:
                logger.info(f"Using organizer's oldest billing profile {billing_profile.id} for event {event_id}")

        if not billing_profile:
            logger.warning(f"No billing profile found for organizer {event.created_by_id}")
            return {"error": "No billing profile", "organizer_id": event.created_by_id}

        # 4. Get active pricing tier for this event type
        pricing_tier = db.execute(
            select(PricingTier)
        ...
        while True:
            check = db.execute(
                select(PlatformInvoice).where(PlatformInvoice.invoice_number == invoice_number)
            ).scalar_one_or_none()
            if not check:
                break
            invoice_number = generate_invoice_number()
        ...
        try:
            stripe_result = stripe_billing_service.create_invoice(
                invoice=invoice,
            ...
            if stripe_result.get("stripe_invoice_id"):
                ...
                logger.info(f"Invoice {invoice.invoice_number} created but Stripe not configured")
                return {
                    "success": True,
                    "invoice_id": invoice.id,
                    "invoice_number": invoice.invoice_number,
...
def check_overdue_invoices(self):
    ...
    try:
        return _sync_check_overdue()
    except Exception as e:
        logger.error(f"Failed to check overdue invoices: {e}")
        raise


def _sync_check_overdue() -> dict:
    """Sync implementation of overdue invoice check."""
    with SyncSessionLocal() as db:
        now = datetime.now(timezone.utc)
