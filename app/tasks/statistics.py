...
def recalculate_event_stats(self, event_id: int):
    ...
    try:
        ...
        try:
            result = loop.run_until_complete(_async_recalculate_event_stats(event_id))
            return result
        finally:
            loop.close()
...
async def _async_recalculate_event_stats(event_id: int) -> dict:
    """
    Async implementation of event stats recalculation.
    ...
    async with CelerySessionContext() as session_maker:
        async with session_maker() as db:
            ...
            ta_participants_result = await db.execute(
