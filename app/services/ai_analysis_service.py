...
class AiAnalysisService:
    ...
    def create_pending_analysis_sync(
        self, db: Session, catch_id: int
    ) -> CatchAiAnalysis:
        """Sync version of create_pending_analysis for Celery tasks."""
        analysis = CatchAiAnalysis(
            catch_id=catch_id,
            status=AiAnalysisStatus.PENDING.value,
        )
    ...
    def analyze_catch_sync(
        self, db: Session, catch_id: int
    ) -> dict:
        """Sync version of analyze_catch for Celery tasks."""
        start_time = datetime.utcnow()
        ...
        try:
            species_result = self._detect_species_sync(db, catch)
            if species_result:
                results["species"] = species_result
                analysis.detected_species_id = species_result.get("detected_species_id")
                analysis.species_confidence = species_result.get("confidence")
            ...
            anomalies = self._detect_anomalies_sync(db, catch)
            results["anomalies"] = anomalies
            analysis.anomaly_flags = anomalies
            analysis.anomaly_score = self._calculate_anomaly_score(anomalies)

            metadata = self._analyze_metadata_sync(catch)
            results["metadata"] = metadata
            analysis.metadata_warnings = metadata
            ...
            auto_validated = self.perform_auto_validation_sync(db, catch_id, analysis)
            results["auto_validated"] = auto_validated
    ...
    def _detect_species_sync(
        self, db: Session, catch: Catch
    ) -> Optional[dict]:
        """Sync version of _detect_species for Celery tasks."""
        image_url = catch.photo_url
    ...
    def _detect_anomalies_sync(
        ...
        """Sync version of _detect_anomalies for Celery tasks."""
        flags = []
        ...
        if catch.event:
            ...
            rapid_result = db.execute(rapid_stmt)
            rapid_catches = rapid_result.scalars().all()
            if len(rapid_catches) >= 3:
                flags.append({
                    "code": "rapid_submissions",
    ...
    def _analyze_metadata_sync(self, catch: Catch) -> list[dict]:
        """Sync version of _analyze_metadata (no DB calls, reuses logic)."""
        warnings = []
    ...
    def perform_auto_validation_sync(
        self,
        db: Session,
        catch_id: int,
        analysis: CatchAiAnalysis,
    ) -> bool:
        """Sync version of perform_auto_validation for Celery tasks."""
        from app.models.user import UserAccount
        from app.models.catch import CatchStatus
        ...
        moderator_stmt = select(UserAccount).where(UserAccount.email == AI_MODERATOR_EMAIL)
        moderator_result = db.execute(moderator_stmt)
        ai_moderator = moderator_result.scalar_one_or_none()
