    ...
    if "top_x_overall" in scoring_code:
        ...
        for i, catch in enumerate(sorted_catches):
            ...
            details.append({
                "catch_id": catch.id,
                "fish_id": catch.fish_id,
                "fish_name": catch.fish.name_en if catch.fish else "Unknown",
                "length": catch.length,
                "points": points,
...
async def _get_individual_leaderboard(
    ...
    for user_id, catches in user_catches.items():
        ...
        # Best catch — always the longest, regardless of min length
        best_catch = max(catches, key=lambda c: c.length) if catches else None

        # Average catch length — based on scored catches only
        scored_catches = score_data["scored_catches"]
        scored_total_length = sum(c.length for c in scored_catches)
        average_catch = round(scored_total_length / len(scored_catches), 2) if scored_catches else 0.0
        ...
        entry = {
            ...
            "best_catch_species": best_catch.fish.name_en if best_catch else None,
    ...
    # Sort with 6-level tiebreaker chain
    # 1. Total points (highest), 2. Counted catches (highest), 3. Species count (highest)
    # 4. Best catch (highest), 5. Average scored catch (highest), 6. First catch time (earliest)
        ...
        for m in movements:
            if m.team_id is not None:
                ...
                team_movements.append({
                    ...
                    "catch_fish": m.catch.fish.name_en if m.catch and m.catch.fish else None,
        ...
        "rejected_catches": [
            {
                "catch_id": c.id,
                "fish_id": c.fish_id,
                "fish_name": c.fish.name_en if c.fish else "Unknown",
                "length": c.length,
                "photo_url": c.photo_url,
